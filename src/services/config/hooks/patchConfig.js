...
const _handleAsyncError = (err: ConfigError, service) => {
  ...
  if (
    ...
    try {
      ...
      if (_configPathIsNotNullOrUndefinedAndWillChange('mongo.dockerized', nextConfig)) {
        _.set(err, ['errors', 'config.mongo.dockerized'], 'error exists in Mongo Docker config');
      }
