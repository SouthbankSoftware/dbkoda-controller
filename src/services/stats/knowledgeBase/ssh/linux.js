...
const commandParsers = {
  cpu: ({ output, samplingRate }) => {
    ...
    if (items.length >= 17) {
      ...
      let cpuUsage =
        data.details.cpu.us + data.details.cpu.sy + data.details.cpu.wa + data.details.cpu.st;
      if (cpuUsage > 100) {
        cpuUsage = 100;
      }
      data.cpu = { usage: cpuUsage, runQueue: data.details.procs.r };
      const totalMemory =
        data.details.memory.swpd +
        data.details.memory.buff +
        data.details.memory.cache +
        data.details.memory.free;
      let usedMemory =
        data.details.memory.swpd + data.details.memory.buff + data.details.memory.cache;
  ...
  network: ({ output, samplingRate }) => {
    ...
    return {
      ...
      value: { network: { upload, download, samplingRate } }
    };
  }
};
...
const common = {
  ...
  parse: (key, output, samplingRate) => {
    // define the parse command output logic, the key is defined in knowledge base
