 ...
 * @Last modified time: 2018-06-13T10:47:29+10:00
 *
...
describe('test pty parser', () => {
  ...
  it('test csi parser for J Clears part of the screen. ', () => {
    ...
    assert.equal(parser.buffers[0], '23');
  });
  ...
  it('test parser for parsing normal string with enter at the last', () => {
    ...
    assert.equal(parser.buffers[0], 'MongoDB shell version v3.4.0');
    assert.equal(parser.buffers[1], '');
    ...
    assert.equal(parser.buffers[0], 'connecting to: mongodb://localhost');
    assert.equal(parser.buffers[8], 'show dbs');
    assert.equal(parser.buffers[9], '');
  });
  ...
  it('test parse FEEDLINE \n character', () => {
    ...
    assert.equal(parser.buffers.length, 2);
    assert.equal(parser.buffers[0], 'first line ');
    assert.equal(parser.buffers[1], ' second line');
  });
  ...
  it('test parse carriage return \r character', () => {
    ...
    assert.equal(parser.buffers[0], ' second line');
  });
});
